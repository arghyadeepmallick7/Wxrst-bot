...
Select a Python egg/image with Python 3.11+ (Python 3.14.7 is fine when supplied by the host).



Upload the files above and your existing config.json and .env.
...
Start or fully restart the server and check the console for Logged in as and Synced ... slash command(s). Commands, including /vcnotify, are synced directly to GUILD_ID and should appear immediately after the restart. If GUILD_ID is empty, the bot syncs directly to every guild it is currently in.



The bot first looks for system ffmpeg, then the imageio-ffmpeg binary installed from requirements. If your host has FFmpeg at another path, add FFMPEG_PATH=/path/to/ffmpeg to .env and restart.
...
In Bot, enable Server Members Intent and Message Content Intent. Message Content Intent is also required for .[text] TTS; without it Discord delivers no readable message text to the bot. Keep Voice States enabled under Gateway Intents so target voice-channel notifications can receive join/leave events.
...
Ensure the bot's server role is above every role it should assign and has access to the target welcome/goodbye and voice channels.
...
Restart the bot and confirm its console says it synced slash commands.



Run /vcnotify set, select the watched voice channel, then select the text channel where alerts should be sent. This command enables notifications.



Join the watched channel, leave it, then move into and out of it. Each action should produce exactly one alert. Use /vcnotify status to verify the saved channels if no alert arrives.
...
Confirm Message Content Intent is enabled in the Discord Developer Portal and restart the bot after enabling it.



Join a voice channel in which the bot has Connect and Speak permissions.



Send exactly .[hello] in a normal server text channel. The bot joins (if needed), generates speech, and plays it through the same connection used for music.



If the bot reports a speech-generation failure, permit outbound HTTPS access from the host for gTTS. If it reports FFmpeg unavailable, keep imageio-ffmpeg in requirements.txt or set FFMPEG_PATH.
...
Create a private ticket category, a private transcript channel, and a private ticket-log channel. Give staff access; do not give regular members access to the transcript/log channels.



Run /ticketconfig channels and select those three channels.



Run /ticketconfig supportrole once for every support role. Administrators always have ticket access.



Optionally set limits with /ticketconfig limits, branding with /ticketconfig panel, features with /ticketconfig features, and custom buttons with /ticketconfig category.



Run /ticketsetup in the channel that should contain the public support panel.
...
Run /ticketconfig channels and set a private transcript channel.



In that channel, grant the bot View Channel, Send Messages, Attach Files, and Read Message History.



Run /ticketconfig limits and set auto_delete_minutes to a value above 0 (for example, 5).



Run /ticketconfig status to confirm both settings.



For an already closed ticket, run /ticket retrytranscript inside that ticket. Once delivery succeeds, auto-delete is scheduled using the configured delay.
...
Run /departure setinvite with your server invite, then customize each template with /departure setmessage.



Use /departure settings to choose which of leave, kick, and ban send DMs.



Run /departure test for each type; it safely DMs only the administrator who runs the command.



Run /departure enable when you are ready. Use /departure disable to pause all departure DMs.
...
Join a voice channel and run /join. The bot should join that channel. Run /leave afterward; it should leave and reset the queue.



Join voice and run /play never gonna give you up or /play <YouTube URL>. The bot should join automatically (if needed), extract audio without downloading a video file, and announce the title/requester.



Run /play with two more searches, then /queue. The first title is Currently Playing and the rest appear under Up Next. Test /shuffle and /remove 2.



While playing, run /pause, /resume, /skip, and /stop. Skip advances to the next waiting track; stop clears everything. Test /loop to repeat only the current track and /volume 25 to set 25% volume.
