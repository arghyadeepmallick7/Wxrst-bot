...
# ---------------------------------------------------------------------------
# Persistent voice-channel controls
# ---------------------------------------------------------------------------
# /join saves the selected channel. The bot remains there when it is empty,
# and reconnects after a process or Discord voice connection interruption.
# Only /leave removes this saved selection.

persistent_voice_reconnect_tasks: dict[int, asyncio.Task[None]] = {}
...
async def maintain_voice_connection(guild: discord.Guild, delay: float = 0) -> None:
    """Reconnect until /leave clears the saved voice-channel selection."""
    if delay:
    ...
    while (channel_id := persistent_voice_channel_id(guild.id)) is not None:
        ...
        if not isinstance(channel, discord.VoiceChannel):
            logger.warning(
                "Saved persistent voice channel %s is missing in guild %s.",
                channel_id,
                guild.id,
            )
            return
        ...
        try:
            # A dropped VoiceClient must be cleaned up before discord.py can
            # establish a fresh connection to the saved channel.
...
def schedule_voice_reconnect(guild: discord.Guild, delay: float = 5) -> None:
    ...
    if task is None or task.done():
        persistent_voice_reconnect_tasks[guild.id] = asyncio.create_task(
            maintain_voice_connection(guild, delay)
        )
...
async def join_voice(interaction: discord.Interaction) -> None:
    if not require_guild_admin(interaction):
        await interaction.response.send_message(
            "Only administrators can use /join.", ephemeral=True
        )
        return
    ...
    if voice_state is None or not isinstance(voice_state.channel, discord.VoiceChannel):
        await interaction.response.send_message(
            "Join a normal voice channel first, then use /join.", ephemeral=True
        )
        return
    ...
    if not permissions.connect or not permissions.speak:
        await interaction.response.send_message(
            "I need View Channel, Connect, and Speak permissions in that VC.",
            ephemeral=True,
        )
        return
    ...
    try:
        ...
        await interaction.response.send_message(
            f"✅ Joined {channel.mention}. I will stay until an administrator uses /leave."
        )
    except (discord.ClientException, discord.Forbidden, discord.HTTPException, asyncio.TimeoutError) as error:
        logger.warning("Could not join persistent voice channel: %s", error)
        await interaction.response.send_message(
            f"I could not join that VC: {error}", ephemeral=True
        )
...
async def leave_voice(interaction: discord.Interaction) -> None:
    if not require_guild_admin(interaction):
        await interaction.response.send_message(
            "Only administrators can use /leave.", ephemeral=True
        )
        return

    # Clear the saved channel before disconnecting so automatic recovery
    # cannot reconnect the bot after an intentional /leave.
    ...
    if voice_client is None or not voice_client.is_connected():
        await interaction.response.send_message(
            "I was not connected to a voice channel. Persistent VC mode is now off.",
            ephemeral=True,
        )
        return
...
async def on_voice_state_update(
    ...
    # Reconnect only when /join previously saved a persistent VC channel.
